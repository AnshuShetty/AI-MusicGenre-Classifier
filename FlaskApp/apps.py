...
# Flask app setup
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": ["http://localhost:3000", "http://localhost:3001"]}})  # Allow React frontend to communicate

# Load the trained model
MODEL_PATH = './Trained_model(indian-30).h5'
model = tf.keras.models.load_model(MODEL_PATH)

# Genre labels - map model output to actual genre names
# genre_labels = ['blues', 'classical','country','disco','hiphop','jazz','metal','pop','reggae','rock' ]

genre_labels = ['bhajans', 'bolly_rap', 'bolly_romantic', 'bollypop', 'carnatic', 'ghazal', 'semiclassical','sufi']

# MongoDB setup
MONGO_URI = "mongodb://localhost:27017/MusicPlayer"
client = MongoClient(MONGO_URI)
db = client["MusicPlayer"]  # Replace with your database name
